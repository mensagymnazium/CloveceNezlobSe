	...
	public class MojeStrategieJenda : HerniStrategie
	{
		...
		public override HerniRozhodnuti? DejHerniRozhodnuti(Hrac hrac, int hod, IHerniInformace informace)
		{
			...
			for (int i = 0; i < hodnoty.Count; i++)
			{
				...
				if (hra.HerniPlan.ZjistiCilovePolicko(figurkyKtereMuzuHrat[i], hod).JeObsazeno())
				{
					if (hra.HerniPlan.ZjistiCilovePolicko(figurkyKtereMuzuHrat[i], hod).ZjistiFigurkyHrace(hrac) == hrac)
						hodnoty[i] -= vyhozeniSebe;
					else
						hodnoty[i] += vyhozeniSoupere;
				}
			}
